...
def write_to_gcs(path: Path, dataset_file: str, color:str) -> None:
    """
    ...
    gcp_cloud_storage_bucket_block.upload_from_path(
        from_path=path,
        to_path=f"data/{color}/{dataset_file}.csv.gz"
    )


@flow(log_prints=True)
def extract_to_gcs(year: int, month: int, color: str) -> None:
    """
    ...
    write_to_gcs(path, dataset_file, color)
...
def etl_data_flow(
    ...
    for month in months:
        extract_to_gcs(year, month, color)


if __name__ == '__main__':
    months = list(range(1, 13))
    year = 2019
    etl_data_flow(months=months, year=year)
